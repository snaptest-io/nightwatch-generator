...
module.exports = function () {
  ...
  var componentCode = generateComponentCode(this.components, this.ActionInfo);
  this.tests.forEach((test) => test.content = generateTestCode(test, this.components, this.ActionInfo));
  ...
  // Create driver file.
  var driverString = fs.readFileSync(__dirname + "/./templates/snaptest-nw-driver.js", 'utf8');
...
function generateComponentCode(components, ActionInfo) {

  var generatedCompCode = "";

  components.forEach((component) => {

    var actionListCode = generateActionList(component.actions, components, ActionInfo);
...
function generateTestCode(test, components, ActionInfo) {
    ...
    ` 
      ...
      module.exports = {
        "${test.name.replace(/['"]/g, "\\'")}" : function (browser) {
          ...
          browser${generateActionList(test.actions, components, ActionInfo)}
            .end();
        }
      };
    `;
